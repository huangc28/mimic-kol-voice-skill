...
1. **`handle`** (required): The KOL's X handle (e.g., `elonmusk`). If missing, ask: _"請提供目標 KOL 的 X handle（例如 elonmusk）"_
2. Check if `artifacts/kol/<handle>/corpus.v1.jsonl` already exists. If yes, ask: _"已經有 @{handle} 的語料庫了（{count} 篇）。要用現有的還是重新收集？"_

### For draft generation (Skill C + D):
1. **`topic`** (required): What the post is about. If missing, ask: _"請提供你想寫的主題"_
2. **`facts`** (optional but recommended): Specific facts/data points. If missing, ask: _"有沒有具體事實要放進推文？沒有的話我會根據主題生成"_
...
|---|---|
| "分析 @handle 的風格" | Run Skill A → Skill B only |
| "用 @handle 的風格寫..." | Run full pipeline (A → B → C → D), skip A+B if profile exists |
| "重新收集 @handle" | Force re-run Skill A (ignore existing corpus) |
| "幫我產生推文" | Ask for handle + topic, then run C → D |
